...
################################################################################
setwd("C:/Users/inpe-eba/SISMOI/Indicadores/SPI")
...
#calculo da precipitação acumulada para 12 meses
pr_rcp45_acc = byapply(pr_rcp45_norm, 12, apply, 1, sum)

pr_rcp85_acc = byapply(pr_rcp85_norm, 12, apply, 1, sum)
...
# calculo do Indice SPI

SPI_rcp45 = as.data.frame((pr_rcp45_acc - pr_rcp45_mean) / pr_rcp45_sd)

SPI_rcp85 = as.data.frame((pr_rcp85_acc - pr_rcp85_mean) / pr_rcp85_sd)
...
SPI_rcp45 = SPI_rcp45[c(141,142,1:140)]

SPI_rcp85 = SPI_rcp85[c(141,142,1:140)]
...
names(SPI_rcp45) <- c("lon","lat", paste(dates,as.character(), sep="_"))

names(SPI_rcp85) <- c("lon","lat", paste(dates,as.character(), sep="_"))

# Exportar o data frame como tabela CSV

write.csv(SPI_rcp45, file = "SPI_rcp45.csv")

write.csv(SPI_rcp85, file = "SPI_rcp85.csv")
